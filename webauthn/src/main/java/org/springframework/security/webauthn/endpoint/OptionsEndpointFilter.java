...
public class OptionsEndpointFilter extends GenericFilterBean {
	...
	OptionsResponse processRequest(HttpServletRequest request) {
		...
		Options options = optionsProvider.getOptions(request, loginUsername, null);
		List<WebAuthnPublicKeyCredentialDescriptor> credentials =
				options.getCredentials().stream().map(WebAuthnPublicKeyCredentialDescriptor::new).collect(Collectors.toList());
		return new OptionsResponse(
				options.getRelyingParty(),
				options.getUser(),
				options.getChallenge(),
				options.getPubKeyCredParams(),
				options.getRegistrationTimeout(),
				options.getAuthenticationTimeout(),
				credentials,
				options.getAuthenticatorSelection(),
				options.getAttestation(),
				options.getRegistrationExtensions(),
